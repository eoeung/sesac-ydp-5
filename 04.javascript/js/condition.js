...
function loginUser() {
  ...
  // inputId에 빈 값이 입력됐다면, '아이디 입력 안함' 문구 반환
  // 두 경우가 아니라면(inputId가 틀렸을 때), '아이디 틀림' 문구 반환
  if (userId === inputId) {
    if (userPw === inputPw) {
      return '로그인 성공';
    } else {
...
// Quiz. 학점 계산기를 switch문으로
// ※ 0 ~ 100 이외의 숫자는 입력되지 않음
let score = parseInt(prompt('학점을 입력해주세요.') / 10, 10);
// console.log(score);
